...
| `Norm.lean`    | The `thales_norm` simp set (attribute registered in `NormAttr.lean`): normalization lemmas plus attribute-tagged models — what lets pure-looking code shed its monadic wrapping so the closers see bare arithmetic. The set's knowledge is dual-tagged for grind (`ballIco_iff` and the models), so the grind rung can open bounded ∀s and unfold models itself. Vanilla Lean carries no binary64 arithmetic theory, so this is where hand-proven `Float` facts land as they are acquired — each recurring residual goal is one lemma to add.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `Prove.lean`   | `#thales_prove` elaboration: `elabProp` builds the `Prop` (bounded binders → nested `ballIco`, unbounded int/nat → plain `∀`s, `number` binders → a `Float` ∀ plus its bounds as hypotheses, `ts.imp` guards → `= pure true` hypotheses in front of the conclusion (a thrown guard fails the hypothesis, so it excludes the assignment where the refuter would report `threw`), `≡` → `TsM Float` equality, boolean islands → `= pure true`), then a four-rung ladder — kernel decide and compiled evaluation for all-bounded domains, then simp/omega and grind on the residual — kernel-checks every proof via `addDecl` (the evaluation tier admits an axiom instead, which is why reasons read their trust level off the theorem's axioms rather than asserting it). The rungs split the per-annotation `thales.heartbeats` budget (bounded: a quarter each; unbounded: half to each symbolic rung), each under its own heartbeat window, so an early blowout (elaborator- or kernel-side) falls through to the next rung instead of consuming the annotation. Compiled evaluation cannot be interrupted by any budget, so it is capped separately by `thales.maxEvaluatedElements` against the size of the enumeration it would perform. |
