...
| `Prove.lean`   | `#thales_prove` elaboration: `elabProp` builds the `Prop` (bounded binders → nested `ballIco`, unbounded int/nat → plain `∀`s, `≡` → `TsM Int` equality, boolean islands → `= pure true`), then a two-rung ladder — decide for all-bounded domains, simp/omega for the rest — kernel-checks every proof via `addDecl`, all under the per-annotation `thales.heartbeats` budget. |
