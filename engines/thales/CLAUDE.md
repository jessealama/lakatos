...
- `Theorem` — a rung of the ladder succeeded; the proof was added to the environment. The reason says which trusted base it rests on, read off the theorem's own axioms: kernel-checked, or admitted from a compiled evaluation that trusts the compiler and the host's floating point unit.
