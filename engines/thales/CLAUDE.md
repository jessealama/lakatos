...
| `Norm.lean`    | The `thales_norm` simp set (attribute registered in `NormAttr.lean`): normalization lemmas plus attribute-tagged models — what lets pure-looking code shed its monadic wrapping so omega sees bare Int arithmetic.                                                                                                                                                                                                                                                                                                                                                    |
...
| `Prove.lean`   | `#thales_prove` elaboration: `elabProp` builds the `Prop` (bounded binders → nested `ballIco`, unbounded int/nat → plain `∀`s, `≡` → `TsM Int` equality, boolean islands → `= pure true`), then a two-rung ladder — decide for all-bounded domains, simp/omega for the rest — kernel-checks every proof via `addDecl`. On bounded domains the rungs split the per-annotation `thales.heartbeats` budget, each under its own heartbeat window, so a decide blowout (elaborator- or kernel-side) falls through to the generic rung instead of consuming the annotation. |
...
- `Theorem` — either rung of the ladder succeeded (decide, or simp/omega); the proof was added to the environment, so it is kernel-checked, not just elaborated.
