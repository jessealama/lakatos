...
| Module         | Role                                                                                                                                                                                                                                                     |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Syntax.lean`  | `declare_syntax_cat` grammar for the core constructors (`ts_type`/`ts_expr`/`ts_stmt`/`ts_param`/`ts_binder`/`ts_prop`), the `ts_def` declaration and `#thales_prove` command. Parsing never fails on unknown _meaning_ — that's an elaboration failure. |
...
| `Prove.lean`   | `#thales_prove` elaboration: `elabProp` builds the `Prop` (binders → nested `ballIco`, `≡` → `TsM Int` equality, boolean islands → `= pure true`), `attemptDecide` proves via `Meta.mkDecideProof` and `addDecl`s the theorem so the kernel checks it.   |
| `Verdict.lean` | `Identity` (the `[file, function, property]` triple shared with the refutation engine), `Verdict`, and the sentinel-framed single-line JSON emitter.                                                                                                     |
...
- `Theorem` — `decide` succeeded; the proof was added to the environment, so it is kernel-checked, not just elaborated.
...
- `GaveUp` — the property elaborated but `decide` failed, including falsity with no witness to ship (zero binders, or witness extraction degraded).
...
This six-status set must match `PROVE_STATUSES` in root `src/envelope.ts`; the root suite pins the check script's copy against it (`tests/verdict-contract.test.ts`). Don't add or rename a status in one place only.
...
- `scripts/check-transcriber.js` — end-to-end: transcribes `tests/fixtures/tracer.ts` with the _built_ frontend (root build required), runs the artifact, asserts expected verdicts.
...
- **One verdict line per annotation, always** — even for failures the elaborator can see. The check scripts enforce ordering and framing.
