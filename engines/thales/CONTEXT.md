...
**Model**:
The Lean `def` an emitted declaration becomes, under the `TsModel` namespace and tagged `@[js_norm, grind]` so the symbolic rungs can unfold it (a class becomes a `TsModel` structure with a `construct` and one def per getter and method). Results, locals, and fields are typed `Float` — IEEE-754 binary64, the type a TypeScript `number` actually holds — over the `JsM` monad; a parameter may also be an instance of a modeled class or a keyword union over the tagged `JsVal` domain. `int`/`nat` binders still quantify over the mathematical integers and coerce in at the call boundary, which is exactly why an interval reaching past the safe-integer range is refused: outside it the coercion stops being injective. A declaration that fails to model is classified frontend-side instead, with the unmapped construct recorded.
