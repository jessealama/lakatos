...
2. **Codegen** (`codegen.ts` + `emit.ts`) — per source file: mirror path under `.pabst/` (source extension kept: `a.ts` → `.pabst/a.ts.pabst.test.ts`), then string-build a vitest/`@fast-check/vitest` test file: one `test.prop` per spec, arbitraries rendered from binders by `domains.ts` (`arbitraryFor`, on top of lemma's bounds helpers), a fixed 32-bit seed (`seed.ts`, `--seed` to reproduce).
...
- **Generated code is disposable.** `.pabst/` is regenerated every run and never hand-edited; determinism comes from the seed.
