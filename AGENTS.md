...
This project uses **bd** (beads) for agent-local task tracking; GitHub Issues is the issue tracker. Run `bd prime` for full workflow context.
...
- Persistent project memory stays in the agent's own memory files, not `bd remember`; do not create ad hoc memory files in the repo.
