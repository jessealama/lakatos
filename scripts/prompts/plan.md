...
5. Add a human gate only in two cases: the plan contradicts a decision recorded in the #376 body, or it changes a shape an earlier merged slice already fixed (the AST, the value domain, the heap, the completion type, the monad, the ESTree schema). Then `bd gate create --type human --blocks <implement bead> --reason "<what is contradicted or changed, in one sentence>"` and say in the plan's Decisions section that you did. Every other decision the issue leaves open is yours to make: record it in the Decisions section, where the review step checks it, and add no gate. New shapes that later issues will inherit are not a reason to gate; a wrong one is caught in review and costs one slice, a gate costs the maintainer a round trip every time.
