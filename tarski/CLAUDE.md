...
This file provides guidance to Claude Code (claude.ai/code) when working in the tarski package.
...
lake build TarskiTest               # every Lean test under Test/
...
| `Js/Binders.lean`                                                     | Binder-domain meaning: `floatInf` and `ballIco` (bounded ∀ over `[lo, hi)`) with its `Decidable` instance. The witness scans live in the prover, not here: lakatos owns the search, the library owns the meaning.                                                                                                                                                                                                                                                                                                            |
