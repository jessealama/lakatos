...
object-literal directories #395 added, and the seven `Symbol`, `JSON`,
and `*Error` directories #392 added — and
...
#382, on #388, on #384, on #393, on #389, on #395, or on #392: no failure
...
on #392 are gone, and no row has replaced them under that number.

The 1,473 failures below, by owner: 749 what #376 excludes (`eval`,
...
source text with a regular expression), 207 the String wrapper (#391),
180 the global object (#487), 150 the transcendental `Math` members
(#434), 62 the rest of `Array.prototype` and `@@species` (#390), 57 the
global `isNaN` and `isFinite` (#441), 39 iterators (#394), 13
`Math.clz32`/`imul` (#440), 5 `Math.random` (#445), and 11 the five filed
defects (#436, #460, #496, #499, #512).

**The two `Object` and `Function` directories are 2,912 pass and 638
fail**, against 2,761 and 725 before this slice: the 80 rows that waited
on `Symbol` are gone, and so are the 30 that waited on `JSON`. Their 638
are the same list with those two cleared: 287 `eval`, `Date`, `RegExp`,
`Proxy`, `Reflect`, and typed arrays (#376); 143 the global object, whose
`this` at top level a third of `Object`'s older tests reach for (#487);
122 the String wrapper (#391); 40 the rest of `Array.prototype`, which
the order tests reach through `map` and `indexOf` (#390); 36 iterators
...
`test/harness` is 60 pass and 19 fail, against 49 and 23: `compareArray`
and `propertyHelper` reach symbols now, and what is left of them is
`Array.prototype` (8), the global object (6), typed arrays (4), and
`Date` (1).

**The seven directories this slice added are 325 pass, 132 fail, and 52
unsupported**: `Symbol` 68/9/19, `JSON` 109/42/14, `Error` 48/37/8,
...
`Array.prototype` and `@@species` (#390), 6 the String wrapper (#391), 3
the iterator protocol, which is `AggregateError`'s `errors` read from a
user iterator (#394), and 2 a defect this slice found and filed (#512).
**Not one is a symbol, a JSON text, or an `Error` member answering the
wrong thing.**
...
| `test/built-ins/JSON/stringify`                                       | 5     | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
| `test/built-ins/JSON/stringify`                                       | 1     | builtin      | the rest of `Array.prototype` is absent, `toString` among it                      | #390  |
| `test/built-ins/JSON/stringify`                                       | 1     | builtin      | a Lean `String` cannot hold a lone surrogate                                      | #391  |
...
| `test/built-ins/Object`                                               | 6     | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
...
| `test/built-ins/Object/create`                                        | 12    | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
...
| `test/built-ins/Object/fromEntries`                                   | 2     | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
...
| `test/built-ins/Object/getOwnPropertySymbols`                         | 1     | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
...
| `test/built-ins/Object/prototype/toString`                            | 3     | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
...
| `test/harness`                                                        | 8     | builtin      | the rest of `Array.prototype` is absent, `toString` among it                      | #390  |
...
| `test/language/expressions/object`                                    | 1     | builtin      | the String wrapper object and `String.prototype` are absent                       | #391  |
